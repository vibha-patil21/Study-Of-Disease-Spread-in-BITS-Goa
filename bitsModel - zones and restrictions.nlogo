;HOSTEL ZONES,
; RESTRICTIONS MESS,CLASS ( COUNT-STUDENTS/LIMIT)
extensions [gis csv]
...
to setup-gis
  ...
  set hostel-boundary-dataset gis:load-dataset "maps/hostels_zone.shp"
  ...
  set messA gis:find-one-feature mess-dataset "MESS" "A"
  set messD gis:find-one-feature mess-dataset "MESS" "D"
  set messC gis:find-one-feature mess-dataset "MESS" "C"
end
...
to create-pop
  ...
  print(id-list)
  set zone-list ["A" "B" "C" "D" "E" "F" "G" "H"]
  let n length id-list
  create-students n
  [
    let current-id item 0 item who id-list
    let current-hostel item 1 item who id-list
    let pick-zone one-of zone-list
    let hostel-zone word current-hostel pick-zone
    let hostel_vf gis:find-one-feature hostel-boundary-dataset "HOSTELNUM" hostel-zone
    ...
    ;let temp-mess item 2 item who id-list
    ;let mess-zone word temp-mess pick-zone
  ...
  [
    ...
    [
     ...
     if hour mod 4 = 0
      ...
      if hour != mess-duration1 or hour != mess-duration2 or hour != mess-duration3
      [
        ask students with [infected? and curr-location = hostel] [expose-people self]
      ]
      if hour mod 4 = 0
      [
...
to go-to-mess ;[student1]
  ...
  foreach [1 2 3 4] [
   ...
   ask students with [infected? and curr-location = messA] [expose-people self]
...
to expose-students [infected-student]
  ...
  set students-at-same-location students with [ curr-location = infected-location ]
...
to go-to-class
  ...
  foreach matchingSublists [
    ...
    let new-location item 3 x
    let class_vf gis:find-one-feature classroom-boundary-dataset "location" new-location
    let count-students count students with [curr-location = class_vf]
    let limit item 5 x
    print(count-students)
    ;print(limit / count-students)
    if count-students > 0
    [
      if count-students / limit > 1
      [
        ask students with [infected? and curr-location = class_vf] [expose-people self]
        ;print(count-students / limit)
        ;print(x)
      ]
    ]
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>export-plot "total cases" (word seed "zones_batchesON.csv")</final>
