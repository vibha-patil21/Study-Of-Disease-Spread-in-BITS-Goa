...
to setup-gis
  ...
  set classroom-dataset gis:load-dataset "maps/classrooms.shp"
  set hostel-dataset gis:load-dataset "maps/hostels.shp"
  gis:set-world-envelope (gis:envelope-union-of
    (gis:envelope-of boundary-dataset)
    (gis:envelope-of classroom-dataset)
    (gis:envelope-of hostel-dataset)
    )

  gis:set-drawing-color black
  gis:draw boundary-dataset 1
  gis:set-drawing-color grey
  gis:draw hostel-dataset 1
  gis:set-drawing-color red
  gis:draw classroom-dataset 1
end
...
to create-pop
  ...
  [
    ...
    let hostel_vf gis:find-one-feature hostel-dataset "HOSTELNUM" current-hostel
...
to expose-people [infected-student]
  ...
  if count (students-here with [ not infected? and not recovered?] in-radius 0.04) != 0
  [
     ask (students-here with [ not infected? and not recovered?] in-radius 0.04)
     [
        if susceptible? = true
        [
          set exposed? true
...
to go-to-class
  ...
  foreach matchingSublists [
    ...
    let class_vf gis:find-one-feature classroom-dataset "location" new-location
    ;print(class_vf)
...
to go-to-class-direct-index
  ...
  [
    ...
    ;print(class_loc)
    let class_vf gis:find-one-feature classroom-dataset "location" class_loc
    ;print(class_vf)
    let loc gis:location-of gis:centroid-of class_vf
    setxy item 0 loc item 1 loc
    set curr-location class_vf
